...
export default function NewSurveyScreen() {
  ...
  useEffect(() => {
    ...
    fetchMobileClients()
      .then((data) => { if (!cancelled) { setClients(data); setClientsError(null); } })
      .catch((err) => { if (!cancelled) setClientsError(err instanceof Error ? err.message : 'Failed to load clients'); })
  ...
  useEffect(() => {
    if (!selectedClientId) { setProjects([]); setSelectedProjectId(null); setSelectedProjectName(null); return; }
    let cancelled = false;
    setProjectsLoading(true);
    fetchMobileClientProjects(selectedClientId)
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {currentStep === 1 && (
            ...
            <>
              ...
              {/* ── Client Picker ──────────────────────────────── */}
              <View style={styles.section}>
                <Text style={styles.label}>Client <Text style={{ color: '#f87171', fontWeight: '700' }}>*</Text></Text>
                {clientsLoading ? (
                  <ActivityIndicator color={colors.primary} style={{ marginVertical: 8 }} />
                ) : clientsError ? (
                  <Text style={{ color: '#f87171', fontSize: 14, marginTop: 4 }}>{clientsError}</Text>
                ) : (
                  <TouchableOpacity
                    style={[styles.pickerBtn, selectedClientId && styles.pickerBtnSelected]}
                    onPress={() => setClientModalVisible(true)}
                  >
                    <Text style={[styles.pickerBtnText, selectedClientId && styles.pickerBtnTextSelected]}>
              ...
              {/* ── Project Picker (cascades from client) ─────── */}
              {selectedClientId !== null && (
                <View style={styles.section}>
                  <Text style={styles.label}>Project <Text style={{ color: '#f87171', fontWeight: '700' }}>*</Text></Text>
                  {projectsLoading ? (
                    <ActivityIndicator color={colors.primary} style={{ marginVertical: 8 }} />
                  ) : (
                    <TouchableOpacity
                      style={[styles.pickerBtn, selectedProjectId && styles.pickerBtnSelected]}
                      onPress={() => setProjectModalVisible(true)}
                      disabled={projects.length === 0}
                    >
                      <Text style={[styles.pickerBtnText, selectedProjectId && styles.pickerBtnTextSelected]}>
                        {selectedProjectName ?? (projects.length === 0 ? 'No projects available' : 'Select a project…')}
                      </Text>
                      {projects.length > 0 && <Text style={styles.pickerChevron}>▾</Text>}
                    </TouchableOpacity>
                  )}
                </View>
              )}
              ...
              <Modal visible={clientModalVisible} transparent animationType="fade" onRequestClose={() => setClientModalVisible(false)}>
                <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setClientModalVisible(false)}>
                  <View style={styles.modalSheet}>
                    ...
                    <FlatList
                      ...
                      renderItem={({ item }) => (
                        <TouchableOpacity
                          style={[styles.modalItem, selectedClientId === item.id && styles.modalItemActive]}
                          onPress={() => handleClientSelect(item)}
                        >
                          <Text style={[styles.modalItemText, selectedClientId === item.id && styles.modalItemTextActive]}>
                            {item.name}
                          </Text>
                          {selectedClientId === item.id && <Text style={styles.modalCheck}>✓</Text>}
                        </TouchableOpacity>
                      )}
                      ItemSeparatorComponent={() => <View style={styles.modalSep} />}
                    />
                    <TouchableOpacity style={styles.modalClose} onPress={() => setClientModalVisible(false)}>
              ...
              {/* Project Modal */}
              <Modal visible={projectModalVisible} transparent animationType="fade" onRequestClose={() => setProjectModalVisible(false)}>
                <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setProjectModalVisible(false)}>
                  <View style={styles.modalSheet}>
                    ...
                    <FlatList
                      ...
                      renderItem={({ item }) => (
                        <TouchableOpacity
                          style={[styles.modalItem, selectedProjectId === item.id && styles.modalItemActive]}
                          onPress={() => handleProjectSelect(item)}
                        >
                          <Text style={[styles.modalItemText, selectedProjectId === item.id && styles.modalItemTextActive]}>
                            {item.name}
                          </Text>
                          {selectedProjectId === item.id && <Text style={styles.modalCheck}>✓</Text>}
                        </TouchableOpacity>
                      )}
                      ItemSeparatorComponent={() => <View style={styles.modalSep} />}
                    />
                    <TouchableOpacity style={styles.modalClose} onPress={() => setProjectModalVisible(false)}>
                      <Text style={styles.modalCloseText}>Cancel</Text>
                    </TouchableOpacity>
                  </View>
                </TouchableOpacity>
